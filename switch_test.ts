...
Deno.test("baselineLines includes hostname/domain when set and SSH-only VTY", () => {
  const lines = baselineLines(args({ hostname: "SW-1", domainName: "asei.local" }));
  assert(lines.includes("hostname SW-1"));
  assert(lines.includes("ip domain-name asei.local"));
...
Deno.test("snmpLines emits RO/RW/location/contact and trap host", () => {
  ...
  assertEquals(lines, [
    ...
    "snmp-server location Indy MDF",
    "snmp-server contact noc@asei.com",
...
Deno.test("parseShowVersion extracts hostname, version, model, uptime", () => {
  const sample = [
    ...
    "SW-INDY-MDF uptime is 12 weeks, 3 days, 4 hours, 5 minutes",
    "System returned to ROM by power-on",
    "Model number                     : WS-C2960-24TT-L",
  ].join("\n");
  const f = parseShowVersion(sample);
  assertEquals(f.hostname, "SW-INDY-MDF");
